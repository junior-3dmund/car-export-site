      ...
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-silver">
          ...
          <button onClick={() => setNotifOpen((s) => !s)} aria-label="Notifications" className="relative p-2 rounded-sm bg-steel2 text-silver">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 2a4 4 0 00-4 4v2.586L4.293 11.293A1 1 0 004 12h12a1 1 0 00-.293-.707L14 8.586V6a4 4 0 00-4-4z"/><path d="M8.25 15a1.75 1.75 0 003.5 0"/></svg>
        ...
        <div className="bg-charcoal border-t border-steel2">
          <div className="max-w-6xl mx-auto px-6 py-4 text-silver">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={async () => { await fetch('/api/notifications/read', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) }); setNotifications((prev) => prev.map((p) => ({ ...p, read: true }))); }} className="text-sm text-silver/80">Mark all read</button>
            </div>
