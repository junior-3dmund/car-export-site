      ...
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-silver">
          ...
          <button
            ...
            className="relative p-2 rounded-sm bg-steel2 text-silver"
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-ignition text-charcoal text-[10px] px-1 rounded-md">
                {unreadCount}
              </span>
            )}
          </button>
                  ...
                  <li key={n.id} className={`p-3 rounded-sm border border-steel2 ${n.read ? "bg-steel" : "bg-steel/80"}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="text-silver/70 text-xs mt-2">{new Date(n.timestamp).toLocaleString()}</div>
                      </div>
                      {!n.read && (
                        <button onClick={() => markRead(n.id)} className="text-sm text-ignition">Mark read</button>
                      )}
                    </div>
                  </li>
                ))}
